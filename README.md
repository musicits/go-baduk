...
웹 상대는 **MCTS 탐색 봇**입니다. 수를 실제로 읽어서 둡니다. 9로에서 3초면
약 1만 판을 두어 보고 고릅니다. 추천수와 승률도 나옵니다.

**다만 유단자에게는 상대가 안 됩니다.** 신경망 없이 규칙만으로 하는 탐색이라
19로에서는 같은 시간에 훨씬 얕게 볼 수밖에 없습니다. 실력 있는 분이
제대로 두시려면 **KataGo 를 붙여야 합니다** — 아래를 보세요.
...
| `docs/rules.js` | 규칙을 자바스크립트로 옮긴 것. 웹 전용 |
| `docs/bot.js` | MCTS 탐색 봇. 웹 워커에서 돈다 |
